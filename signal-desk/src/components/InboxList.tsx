...
export default function InboxList({ intels, loading, selectedId, listView, onSelect }: Props) {
  ...
  return (
    <>
      {intels.map(intel => (
        <article
          key={intel.id}
          className={`intel-brief ${PRIORITY_STRIPE[intel.priority]}${selectedId === intel.id ? ' selected' : ''}${intel.status === '已读' ? ' read' : ''}`}
